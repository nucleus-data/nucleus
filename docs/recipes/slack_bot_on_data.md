...
>
> None of the agent-runtime APIs (`ctx.agent`, `nucleus-mcp-server`) ship yet. The MCP server (~500 LOC) lands at v0.5 per [v4.1 §18.4](../../nucleus_architecture_v4.1.md) + [ADR-002 §4.2](../decisions/ADR-002-positioning-decision-2026-05.md). North-star reference, not a step-by-step. Every step carries `NEEDS VERIFICATION`.

> **Target time (v0.5+)**: ~30 min on a working v0.1 stack · **Difficulty**: Senior · **Prereqs**: working v0.1 pipeline (see [`postgres_to_iceberg.md`](./postgres_to_iceberg.md)); Nucleus v0.5+ (Mo 20-28 best-case per [v4.1 §17.2](../../nucleus_architecture_v4.1.md), gated on the Mo 24 founder decision per [ADR-002 §8.3](../decisions/ADR-002-positioning-decision-2026-05.md)); Slack bot token; LLM API key
> **Refs**: [v4.1 §7.3](../../nucleus_architecture_v4.1.md) · [v4.1 §18.4](../../nucleus_architecture_v4.1.md) · [ADR-002 §4.2](../decisions/ADR-002-positioning-decision-2026-05.md)
...
```bash
nucleus mcp serve --port 7331                     # <!-- pre-v0.5; no `mcp` subcommand in nucleus_cli_spec.md §1 -->
...
Either path satisfies the [v4.1 §7.3](../../nucleus_architecture_v4.1.md) sandbox: agent cannot modify Tier 0, cannot commit without human approval (production), cannot access secrets outside declared scope.
...
2. Nucleus runs the Iceberg scan (DuckDB, [v4.1 §5.1](../../nucleus_architecture_v4.1.md)).
...
- **Token leak in audit** — secrets MUST not appear in lineage events per [v4.1 §15.3](../../nucleus_architecture_v4.1.md). Release-blocker bug.
...
Forward-looking; nothing here ships before v0.5 (Mo 20-28 best-case per [v4.1 §17.2](../../nucleus_architecture_v4.1.md); gated on the Mo 24 founder decision per [ADR-002 §8.3](../decisions/ADR-002-positioning-decision-2026-05.md)).

1. **`nucleus mcp serve` CLI subcommand** — not in [`nucleus_cli_spec.md`](../../nucleus_cli_spec.md) §1.
2. **`expose_to_agents=True` decorator argument** — narrative-only in [v4.1 §18.4](../../nucleus_architecture_v4.1.md); not yet typed per [`nucleus_asset_model_spec.md`](../../nucleus_asset_model_spec.md).
3. **`agent.serve_slack(...)` API** — not in [`nucleus_ctx_sdk_spec.md`](../../nucleus_ctx_sdk_spec.md); `ctx.agent` is v0.5+ per [v4.1 §13.2](../../nucleus_architecture_v4.1.md), Slack-binding unspecified.
4. **`ctx.llm` provider DSL** (`"anthropic:claude-3-5-sonnet"`) — v0.5+, no naming convention locked; AI-related APIs flex more than core data APIs per [v4.1 §13.3](../../nucleus_architecture_v4.1.md).
...
7. **Sandbox guarantees for production agents** — [v4.1 §7.3](../../nucleus_architecture_v4.1.md) lists guardrails as design intent; threat model deferred to v0.5.
