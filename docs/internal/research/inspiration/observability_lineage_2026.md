...
> **Target**: `docs/internal/research/inspiration/observability_lineage_2026.md`  
> **Prior art**: `docs/internal/research/openlineage.md` (AMA wire format), `docs/internal/research/sqlglot.md` (lineage API), `docs/internal/research/observability_backends.md` (VictoriaMetrics + Marquez deployment), `docs/internal/research/soda.md` (license boundary)
...
The core 4-noun model (Run, Job, Dataset, Facet) is stable. New behavior lands as new facet schemas, not spec rewrites. See `docs/internal/research/openlineage.md` §1–§4 for full API surface.
...
**Key finding — ilum-cloud Marquez v0.54.0 ships Rust backend.** The API server is rewritten in Rust (Axum + SQLx + tokio). Java backend is preserved as an opt-in fallback via `./docker/up.sh --java`. The Rust path is the default. ~38,000 LOC new Rust across 5 workspace crates. 100% API-compatible with upstream Marquez. This resolves the JVM-sidecar issue from `docs/internal/research/observability_backends.md` §2.3.
...
Detailed API coverage in `docs/internal/research/sqlglot.md` §3–§4.1. Summary for this context:
...
Verified schema v1-2-0 (from `docs/internal/research/openlineage.md` §5.2):
...
Critical finding from `docs/internal/research/soda.md` §1.2: v3.x = Apache-2.0 (last release: `3.5.6`, September 2025); v4.x = Elastic License 2.0 (NOT OSI-approved, anti-cloud-hosting clause). v3.x DuckDB connector requires `duckdb<1.1.0` — incompatible with Nucleus's `duckdb==1.1.3` pin. v4 is out of scope due to ELv2. **Soda is off-table for Nucleus through v0.3.**
...
19. Soda Core license research: `docs/internal/research/soda.md` §1.2
...
26. `docs/internal/research/openlineage.md`, `docs/internal/research/sqlglot.md`, `docs/internal/research/observability_backends.md`, `docs/internal/research/soda.md`
