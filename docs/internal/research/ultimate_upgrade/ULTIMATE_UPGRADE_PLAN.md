...
| 4 | T-1 h cold `pip install nucleus` smoke from fresh venv on different OS | `05_*` §6.5 (Killer #1) | HIGH | S | v0.2.0 launch | PASS | n/a (own scripts) |
...
| **MD-4** | **T-1 h cold `pip install nucleus` smoke from fresh venv on different OS** (per `05_*` §6.5 launch-killer #1 mitigation) | Run `python -m venv /tmp/freshcheck && /tmp/freshcheck/bin/pip install nucleus==0.2.0 && /tmp/freshcheck/bin/nucleus version` on Mac AND Linux. Both EXIT 0 within 60 s. If FAIL → defer launch 24 h. |
