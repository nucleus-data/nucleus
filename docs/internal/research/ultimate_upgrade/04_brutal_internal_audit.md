...
| C11 | "v0.2.0 — released 2026-05-15" | `CHANGELOG.md:31`, `pyproject.toml:16` | Source code in `main`; `pyproject.toml` version = `0.2.0`; CHANGELOG flipped. **But**: git tag `v0.2.0` NOT pushed (founder-gated, `AGENTS.md:58`); PyPI publication NOT executed (founder OIDC pre-registration gate, `AGENTS.md:59`); `pip install nucleus-data` returns the previous wheel until the founder pushes. | **PARTIAL** — released to `main`, but the user-facing "release" (PyPI install) is a future event. |
