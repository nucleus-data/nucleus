...
This connector is THE Bosch ELY beachhead unlock. Per `docs/internal/research/parity_vs_bosch_ely_adb_batch.md`:
...
Docs: https://requests.readthedocs.io/en/latest/api/#exceptions · dlt exceptions: read `dlt/extract/exceptions.py` + `dlt/pipeline/exceptions.py` (no central docs page per `docs/internal/research/dlt.md` §3).
...
Status: PROPOSED  ·  Related: ADR-014, docs/internal/research/dlt.md, this doc §3
...
NEEDS VERIFICATION (medium risk): the FSSpec→PyArrow handler had async-vs-sync edge cases historically (e.g. `aiohttp.ClientResponseError` flagged in `docs/internal/research/gcs_duckdb.md` §7). Smoke-test against Azurite before merge.
...
Per AGENTS.md §0 (beachhead = startup data team 5-20) + `docs/internal/research/parity_vs_bosch_ely_adb_batch.md`:
