...
This means even the AMA — the primary planned v0.5 OTEL consumer per `docs/internal/research/opentelemetry.md` §4.1 — has not yet been wired for telemetry. The §6.6 escape-hatch-counter pathway is not yet built.
...
| Axis | Score |
|---|---|
| ✅ pros | Zero v0.1 callers (A2); PoC #2 deferred the planned table-walker; `docs/internal/research/sqlglot.md` §10 explicitly schedules v0.5+ as the next adoption window. Pin re-enters core in the v0.3 marimo upgrade ADR (`26.0.0 → 26.8.x[c]`) per ADR-012. |
...
**Rationale**: zero v0.1 callers (A2); `docs/internal/research/sqlglot.md` §10 enumerates v0.5+ as the next adoption window; ADR-012 schedules the `26.0.0 → 26.8.x[c]` marimo upgrade for v0.3. Anti-Over-Engineering rule #4: no caller, no pin.
...
> **Pin count revised 2026-05-14**: 22 explicit core pins (was 25; −1 removed msgspec; −2 demoted to extras). Optional extras: `[observability]` 1 pin, `[lineage-advanced]` 1 pin. Rationale: `docs/internal/research/otel_day1_decision.md`.
...
Bump ADR-011 Status line to `AMENDED 2026-05-14 — substrate-presence clarified per docs/internal/research/otel_day1_decision.md`.
...
**`docs/internal/research/ai_hallucinations.md`**: no new entries from this research; doc surfaces zero fabricated APIs.
...
1. **`dlt[sql_database,pyiceberg]==1.26.0` transitive `sqlglot` dependency**: `docs/internal/research/dlt.md` was not re-read this session. If dlt pulls sqlglot transitively, demotion from explicit pin still removes the speculative-pin claim but doesn't shrink installed wheels. Verify: `pip show dlt | grep -i Requires`, or `pip install dlt[sql_database,pyiceberg]==1.26.0 && pip show sqlglot`. Does NOT change the recommendation; sharpens install-footprint claim only.

2. **`opentelemetry-api==1.29.0` transitive deps on Python 3.11**: `Deprecated`, `importlib-metadata`, `typing-extensions` listed by `docs/internal/research/opentelemetry.md` §1 but not re-verified against PyPI metadata for the exact pin. Verify: `pip show opentelemetry-api==1.29.0`. Likely uncontroversial.
