...
| Release | Surface |
|---|---|
| v0.1 | **Env vars + `.env` (gitignored)**. `.gitignore` covers `.env*`, `secrets.toml`, `*.pem`, `*.key`, `credentials.json` (per [`threat_model_v0.md`](../internal/security/threat_model_v0.md) §5.1); `detect-private-key` pre-commit on. URL passwords masked to `<REDACTED>` in logs and `NucleusError.user_message`. |
...
- **In `git log` / `git stash`.** A `.env` accidentally added is still in the reflog. Per [`threat_model_v0.md`](../internal/security/threat_model_v0.md) §10, any push is a rotation event. `gitleaks` / `detect-secrets` pre-commit hook is P1 (§11).
...
- **LLM keys (v0.5+) user-side, never proxied** (per [`threat_model_v0.md`](../internal/security/threat_model_v0.md) §8 #3): no single point of compromise; cost is per-user management.
