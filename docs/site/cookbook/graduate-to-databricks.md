...
2. **Iceberg spec version mismatch is possible.** Nucleus pins `pyiceberg` (see `pyproject.toml` and `docs/internal/compatibility.md`). Databricks runtime ships its own Iceberg reader. If you are on Iceberg spec v3 features (deletion vectors, equality deletes), verify Databricks runtime support before relying on them.
