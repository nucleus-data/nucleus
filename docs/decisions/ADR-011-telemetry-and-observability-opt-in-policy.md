...
|---|---|---|---|
| v0.1 | no-op sink (API-only — `NonRecordingSpan` by default; SDK ships via `pip install nucleus-data[observability]` opt-in) | same | n/a |
...
- `opentelemetry-sdk==1.29.0` — **MOVE** to `[project.optional-dependencies] observability`. Install via `pip install nucleus-data[observability]`. SDK pin only matters once an exporter is configured (per §5 v0.5+ scope).
