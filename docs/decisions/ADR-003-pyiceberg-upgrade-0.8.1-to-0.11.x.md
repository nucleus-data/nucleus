...
> **Related**: ADR-001, ADR-002 §4.2, AGENTS.md §11.13, `docs/research/pyiceberg.md` §2 + §6 + §9, `docs/research/dlt.md` §6
...
**Second forcing function (added 2026-05-13)**: the v0.3 connector framework (`dlt`) requires `pyiceberg>=0.9.1` for its Iceberg destination — `dlt[pyiceberg]==1.26.0` will fail `pip install` resolution against our current `0.8.1` pin (see `docs/research/dlt.md` §6, KEY FINDING). This ADR is therefore a hard prerequisite for **two** downstream workstreams now, not one:
...
What 0.11.x buys, per `docs/research/pyiceberg.md` §B.3: `ExpireSnapshots` API, the O(N²) manifest cache fix, generator-based writes, full ORC read, sort-order evolution, REST scan planning, Python 3.13. Skipping 0.9/0.10 is supported and explicitly endorsed by §B.3.
...
Every 0.11.x feature claim above is sourced from `docs/research/pyiceberg.md` §B.3; upgrade considerations from §9.
...
| v0.3 connector framework (dlt) | 14-20 | `dlt[pyiceberg]==1.26.0` requires `pyiceberg>=0.9.1`; the v0.3 dlt ADR cannot land until this upgrade clears (`docs/research/dlt.md` §6). |
