...
**NEEDS VERIFICATION**: The exact class name `dagster.DagsterExecutionStepExecutionError` and its import path are pinned against `dagster==1.9.5`. If a future minor or major Dagster bump renames the wrapper, the translator imports must be updated and a row added to [`../internal/research/ai_hallucinations.md`](../internal/research/ai_hallucinations.md). Cite of intent: AGENTS.md §11.12 + §11.13.
