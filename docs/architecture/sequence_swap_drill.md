...
> **Companion**: [`../internal/swap/dagster.md`](../internal/swap/dagster.md), [`../internal/swap/duckdb.md`](../internal/swap/duckdb.md), [`../internal/swap/pyiceberg.md`](../internal/swap/pyiceberg.md), [`../specs/nucleus_architecture_v4.1.md`](../specs/nucleus_architecture_v4.1.md) §9, [`../../AGENTS.md`](../../AGENTS.md) §3 #9 + §11.11.
...
A stale swap interface is **worse than no swap interface** — it creates the illusion of safety without the substance. The **drill** keeps it honest. v4.1 §9.3 mandates the quarterly cadence; the drill itself — who does what, in what order, with what acceptance criteria — was previously unspecified. This document closes that gap. The first three swap docs (`../internal/swap/{dagster,duckdb,pyiceberg}.md`) are in place; the founder runs the first drill against them after PoC #1 ships and the v0.1 swap interfaces land.
...
Per v4.1 §9.3 + §19 row 11, **calendar is the floor, never the ceiling**. Each `docs/internal/swap/<component>.md` §1 carries a component-specific subclass (e.g. PoC #4 boot regression for DuckDB; iceberg-spec-v3 lag for PyIceberg). Read both lists together before declaring GREEN.
...
Cost rows in `docs/internal/swap/{dagster,duckdb,pyiceberg}.md` §4 (Dagster ~5 wk, DuckDB ~3 wk, PyIceberg ~7-8 wk) are **drill-validated** estimates already carried in the swap docs — re-validating them is part of every drill, not a separate exercise. Per §8 only 1 full adapter is active at a time during v0.5-v1.0.
...
2. **Interface compiles clean** — `mypy --strict src/nucleus/internal/swap/<component>.py` returns 0 (placeholder until v0.5+; §9 row 3).
...
Per v4.1 §9 + §17.2 (solo-founder runway): ≤ 30 minutes per-component-per-drill, ≤ 4 hours per quarter across all Tier 1/2 components, ≈ 16 hrs/year ≈ ~0.4% of solo-founder capacity. A full-adapter build (when triggered) is 3-8 weeks per `docs/internal/swap/*` §4.
...
3. **`src/nucleus/internal/swap/` + `tests/swap_smoke/` do not yet exist in v0.1.** The Protocol sketches inside `docs/internal/swap/{dagster,duckdb,pyiceberg}.md` §2 are normative; the file layout that hosts them is not yet decided.
...
- [`../internal/swap/dagster.md`](../internal/swap/dagster.md), [`../internal/swap/duckdb.md`](../internal/swap/duckdb.md), [`../internal/swap/pyiceberg.md`](../internal/swap/pyiceberg.md); format templates [`sequence_error_translation.md`](sequence_error_translation.md), [`sequence_ingestion.md`](sequence_ingestion.md), [`sequence_asset_materialization.md`](sequence_asset_materialization.md).

---

*Next: when post-PoC #1 swap interfaces land, run the first drill against [`../internal/swap/dagster.md`](../internal/swap/dagster.md) and post the result as `DRILL-YYYY-MM-DD-dagster.md`. Resolve §9 row 5 (drill log location) before the first drill log lands.*
