...
> **Primary reference**: `docs/research/marimo.md` (component-level deep-dive, 2026-05-13) — read that first for Marimo integration spec. This document synthesizes cross-cutting UX patterns and adoption verdict.
...
**Nucleus implication**: marimo's reactive DAG is per-notebook, in-process, dev-time only. Nucleus's asset graph is global, persistent, Iceberg-backed, Dagster-orchestrated. **These must never be merged.** The `@nucleus.asset` decorator is the boundary. See `docs/research/marimo.md` §2 + §5.3.
...
Full conflict table in `docs/research/marimo.md` §7. ADR sequencing: (1) msgspec upgrade, (2) sqlglot upgrade, (3) Marimo integration ADR + adapter.
