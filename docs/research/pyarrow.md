...
The **only PoC where Nucleus constructs a `pyarrow.Table` itself**, because SQLite (via stdlib `sqlite3`) returns Python tuples and pyiceberg's `Table.append()` requires a `pyarrow.Table` (per [`docs/research/pyiceberg.md`](./pyiceberg.md) §5).
...
After pyiceberg commits a snapshot, the AMA reads back the schema to emit an OL `SchemaDatasetFacet` (per [`docs/research/openlineage.md`](./openlineage.md) §5.1):
...
## §5. Known AI hallucinations (verify before merge — log catches to [`docs/research/ai_hallucinations.md`](./ai_hallucinations.md))
...
When any of these is caught in a PR, log it to [`docs/research/ai_hallucinations.md`](./ai_hallucinations.md) with date, PR, and detection method.
