...
| DB connection failure | `NucleusEngineError` | Underlying `duckdb.ConnectionException` per `docs/research/duckdb.md` §6 |
...
*Last verified: 2026-05-13 against `soda-core==3.5.6` (v3, Apache-2.0) and `soda-core==4.10.0` (v4, ELv2). Re-verify when v0.5 telemetry fires the wrap decision, before pinning, or on any major bump. Log any AI-fabricated Soda APIs to [`docs/research/ai_hallucinations.md`](./ai_hallucinations.md).*
