...
Where caught: `docs/research/openlineage.md` §9 (Worker J research doc, 2026-05-13).
...
AI suggested: `quay.io/minio/minio:RELEASE.2025-10-15T17-29-55Z` as the "terminal OSS release" in `docs/research/minio.md` and propagated to `docker-compose.minio.yml`.
...
Detection: Worker B (ADR-008 storage smoke test) attempted `docker pull` and got "manifest unknown"; cross-checked quay.io tag list + Docker Hub mirror. Fixed in `docker-compose.minio.yml` on 2026-05-13. Source research doc `docs/research/minio.md` also corrected in the same pass.

---

(More entries expected during PoC #1 execution — see `docs/research/dagster.md` §6, `docs/research/pyiceberg.md` §6, `docs/research/duckdb.md` §6 for known-unverified items to investigate.)
...
v1 path still works but warns. `docs/research/openlineage.md` §10 already
catalogued this gotcha — the task prompt regressed it.

**Detection**: Cross-referenced `docs/research/openlineage.md` §10 + the
...
**Caught in**: Research wave for `docs/research/ai_copilot.md` + ADR-015 (AI Copilot chat MVP), error-translation table in §8.
...
back. `docs/research/workbench.md` §3 / §5 / §14 cite the verified URL.
...
`docs/research/workbench.md` §3 / §14 cite the verified URL.
...
'@xyflow/react';`). `docs/research/workbench.md` §4 / §14 cite the
