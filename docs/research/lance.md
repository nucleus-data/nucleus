...
> **AI-drift caveat**: confirm import paths, constructors, and `__cause__` chaining by **actually triggering each exception** against pinned versions. Do NOT register translators from class names alone. Log fabrications in `docs/research/ai_hallucinations.md`.
...
*Next review trigger: v0.5 scoping, or the first PR that adds `pylance` / `lancedb` to `pyproject.toml`. Log hallucinated Lance / LanceDB APIs in `docs/research/ai_hallucinations.md`.*
