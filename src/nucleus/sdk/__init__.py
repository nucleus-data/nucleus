...
Per ``nucleus_architecture_v4.1.md`` §13.2 (Surface Summary) +
``docs/decisions/ADR-013-ctx-materialize-api.md`` (the materialize API)
+ ``nucleus_ctx_sdk_spec.md`` §2 + §12 (frozen surface) +
``nucleus_asset_model_spec.md`` §3 + §10 (asset + check semantics).
...
- :mod:`nucleus.ctx` — the per-asset runtime object passed by the
  ...
  the runtime, never by user code (``nucleus_ctx_sdk_spec.md`` §3).
...
``nucleus.sdk.asset``) per ``nucleus_ctx_sdk_spec.md`` §1+§12.
