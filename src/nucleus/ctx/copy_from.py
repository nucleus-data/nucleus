...
Scope (deliberately minimal — v0 SQLite branch):
    ...
    - Will expand to additional source branches as subsequent PoCs graduate.
...
def _open_catalog(warehouse_dir: Path) -> Catalog:
    ...
    + ``warehouse=file:///...`` as plain string kwargs. Pattern stable.
    """
...
def ingest_sqlite_to_iceberg(
    ...
    *,
    ...
    try:
        ...
        # An inner call (e.g., _open_catalog → NucleusCatalogError) already typed
        # the error per v4.1 §6.4; do not double-translate.
        raise
    except Exception as exc:
        # Route through the canonical translator so specific-class signals from
        # pyiceberg (NoSuchTableError, ValidationError, CommitStateUnknownException)
        # and stdlib (FileNotFoundError, PermissionError, ConnectionError, ...)
        # reach the user as the appropriate NucleusError subtype rather than
        # being flattened to NucleusIOError. Per Phase D verifier IMPORTANT #5
        # (2026-05-14) + AGENTS.md §11.7 + v4.1 §6.4.
