...
def _poly(verts, is_orange=False):
    ...
    pts = [
        (
            x + INSET * (gx - x) / math.hypot(gx - x, gy - y),
            y + INSET * (gy - y) / math.hypot(gx - x, gy - y),
        )
        for x, y in verts
    ]
    if is_orange:
...
def _verify():
    rs = [
        math.hypot(sum(v[0] for v in w) / 3 - w[0][0], sum(v[1] for v in w) / 3 - w[0][1])
...
def _premium(resvg, Image, mark_svg, out):
    ...
    mark = Image.open(BytesIO(bytes(resvg.svg_to_bytes(svg_string=mark_svg, width=360)))).convert(
        "RGBA"
    )
...
def main():
    try:
        ...
        print(f"ERROR: pip install resvg-py Pillow ({e})", file=sys.stderr)
        return 1
    ...
    svg = lambda vb, body: (
        f'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 {vb}" role="img" aria-label="Nucleus">\n  {body}\n</svg>\n'
    )
    ...
    src = Image.open(BytesIO(bytes(resvg_py.svg_to_bytes(svg_string=mark, width=256)))).convert(
        "RGBA"
    )
