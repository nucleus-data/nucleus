...
def _record(
    num: int, label: str, status: str, dur: float, detail: str, results: list[StepResult]
) -> None:
...
def _report(results: list[StepResult], wall: float) -> str:
    ...
    lines = [
        ...
        "=" * 60,
        f"{'Step':<30} {'Status':<13} {'Elapsed':>10}",
        "-" * 60,
    ]
    for r in results:
    ...
    if fails:
        ...
        lines += [
            f"Status: PASS-WITH-SKIPS ({skips} command(s) not yet wired)",
            "",
...
def main(argv: list[str] | None = None) -> int:
    argparse.ArgumentParser(
        description="Walk the v0.1 beachhead chain (AGENTS.md §11.8)."
    ).parse_args(argv)
    ...
    status, detail = _classify(
        rc, out, err, ("nucleus", "duckdb", "polars", "pyiceberg", "dagster")
    )
    ...
    rc, out, err, dur = _run(
        [*nucleus, "ingest", "sqlite:///users.db", "--table", "users", "--as", "raw.users"],
        project_dir,
    )
    ...
    if status5 == "SKIPPED":
        ...
        rc, out, err, dur = _run(
            [*nucleus, "query", "SELECT count(*) as cnt FROM raw.users"], project_dir
        )
