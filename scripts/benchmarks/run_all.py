...
``docs/internal/benchmarks/<date>_baseline.md``.

Usage::

    python scripts/internal/benchmarks/run_all.py                # all five
    python scripts/internal/benchmarks/run_all.py --only b5,b2   # subset
    python scripts/internal/benchmarks/run_all.py --report-only  # re-render markdown only

The orchestrator is intentionally **dumb** — it does no measurement
itself. Each ``b*_*.py`` script writes its own
``docs/internal/benchmarks/_results/<name>.json`` blob; the report is a
...
def _result_path_for(short_name: str) -> Path:
    """Map ``"b5"`` → ``docs/internal/benchmarks/_results/b5_boot_time.json``."""
...
def _render_report(short_names: list[str], total_wall_s: float) -> str:
    ...
    parts.append(
        ...
        > now lives at `scripts/internal/benchmarks/`. Re-run with
        > `python scripts/internal/benchmarks/run_all.py`. Per AGENTS.md §11.13 a regression
    ...
    parts.append(
        ...
        python scripts/internal/benchmarks/run_all.py
    ...
    parts.append("---\n")
    parts.append(f"_Generated by `scripts/internal/benchmarks/run_all.py` on {now_iso()}._\n")
    return "\n".join(parts)
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        "(default: docs/internal/benchmarks/<YYYY-MM-DD>_baseline.md).",
    )
