...
from poc.p1_error_translation.translator import translate  # noqa: E402
...
def test_timeout_error_translates_to_source_connection_error() -> None:
    captured = _run_failing_asset(TimeoutError("read on socket timed out after 30s"))
    out = translate(captured)

    assert isinstance(out, NucleusSourceConnectionError)
    assert "timed out" in out.user_message.lower()
    assert out.__cause__ is captured
    assert "dagster" not in out.rendered().lower()
...
@pytest.mark.skip(
    reason=(
        "Dagster 1.9.5 do_raise overwrites wrapper.__context__ in re-raise path; "
        "test fights Python semantics. See poc/p1_error_translation/PROMOTION_PR_DRAFT.md "
        "§Known issues #1 for the Option A rewrite plan. Mirrors the skip applied at "
        "tests/coordination/test_error_translation.py:273 during Phase A autopilot (A1.2 Option B)."
    )
)
